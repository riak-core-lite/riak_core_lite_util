...
-export([member_status/1, ring_status/1, print_member_status/2, stage_leave/1,
         stage_remove/1, stage_replace/1, stage_resize_ring/1, stage_force_replace/1,
         print_staged/1, commit_staged/1, clear_staged/1, transfer_limit/1,
         pending_claim_percentage/2, transfers/1, add_user/1, alter_user/1, del_user/1,
         add_group/1, alter_group/1, del_group/1, add_source/1, del_source/1, grant/1, revoke/1,
         print_users/1, print_user/1, print_sources/1, print_groups/1, print_group/1,
         print_grants/1, security_enable/1, security_disable/1, security_status/1, ciphers/1,
...
print_member_status(Ring, LegacyGossip) ->
    ...
    {Joining, Valid, Down, Leaving, Exiting} =
        lists:foldl(fun ({Node, Status}, {Joining0, Valid0, Down0, Leaving0, Exiting0}) ->
                            StatusOut =
                            ...
                            {RingPercent, NextPercent} = pending_claim_percentage(Ring, Node),

                            case IsPending of
                                true ->
                                    io:format("~-8s  ~5.1f%    ~5.1f%    ~p~n",
                                              [StatusOut, RingPercent, NextPercent, Node]);
                                false ->
...
claimant_status(Claimant, RingReady) ->
    ...
    case RingReady of
        undefined ->
            io:format("Status:     down~nRing Ready: unknown~n", []);
        _ ->
            io:format("Status:     up~nRing Ready: ~p~n", [RingReady])
    end,
    io:format("~n", []).
...
print_ownership_status({Owner, NextOwner}, Transfers, Down) ->
    io:format("Owner:      ~s~nNext Owner: ~s~n", [Owner, NextOwner]),
    case {lists:member(Owner, Down), lists:member(NextOwner, Down)} of
        {true, true} ->
            io:format("~n"),
...
print_transfer_status({Idx, Waiting, Complete, Status}) ->
    ...
    case Status of
        complete ->
            io:format("  All transfers complete. Waiting for claimant to change ownership.~n");
        awaiting ->
...
unreachable_status(Down) ->
    ...
    io:format("~n", []),
    io:format("WARNING: The cluster state will not converge until all nodes~nare "
              "up. Once the above nodes come back online, convergence~nwill "
              "continue. If the outages are long-term or permanent, you~ncan "
              "either mark the nodes as down (riak-admin down NODE) or~nforcibly "
              "remove the nodes from the cluster (riak-admin~nforce-remove "
              "NODE) to allow the remaining nodes to settle.~n"),
    ok.
...
transfers([]) ->
    try
        ...
        F =
            fun ({waiting_to_handoff, Node, Count}, Acc) ->
...
print_arrowbox(SrcAtom, TargetAtom, Objs, Bytes, Progress) ->
    ...
    ToFrom = riak_core_format:fmt("~25s ~10s ~25s", [Src, ?ARROW, Target]),
    Width = length(ToFrom),
    ...
    try
        case riak_core_claimant:leave_member(Node) of
            ...
            {error, already_leaving} ->
                io:format("~p is already in the process of leaving the cluster.~n", [Node]),
                ok;
            {error, not_member} ->
...
stage_replace(Node1, Node2) ->
    try
        case riak_core_claimant:replace(Node1, Node2) of
            ok ->
                io:format("Success: staged replacement of ~p with ~p~n", [Node1, Node2]),
                ok;
            {error, already_leaving} ->
                io:format("~p is already in the process of leaving the cluster.~n", [Node1]),
                ok;
            {error, not_member} ->
                io:format("Failed: ~p is not a member of the cluster.~n", [Node1]),
                error;
            {error, invalid_replacement} ->
                io:format("Failed: ~p is not a valid replacement candidate.~nOnly newly "
                          "joining nodes can be used for replacement.~n",
                          [Node2]),
                error;
            {error, already_replacement} ->
                io:format("Failed: ~p is already staged to replace another node.~n", [Node2]),
                error
        end
    catch
        Exception:Reason ->
...
stage_force_replace(Node1, Node2) ->
    try
        case riak_core_claimant:force_replace(Node1, Node2) of
            ok ->
                io:format("Success: staged forced replacement of ~p with ~p~n", [Node1, Node2]),
                ok;
            {error, not_member} ->
                io:format("Failed: ~p is not a member of the cluster.~n", [Node1]),
                error;
        ...
        Exception:Reason ->
            lager:error("Forced node replacement failed ~p:~p", [Exception, Reason]),
...
stage_resize_ring(["abort"]) ->
    try
        ...
        Exception:Reason ->
            lager:error("Abort resize ring request failed ~p:~p", [Exception, Reason]),
...
stage_resize_ring(NewRingSize) ->
    try
        case riak_core_claimant:resize_ring(NewRingSize) of
            ok ->
                io:format("Success: staged resize ring request with new size: ~p~n", [NewRingSize]),
                ok;
            {error, not_capable} ->
                io:format("Failed: at least one node is not capable of performing the "
                          "operation~n"),
                error;
            {error, same_size} ->
                io:format("Failed: current ring size is already ~p~n", [NewRingSize]),
                error;
        ...
        Exception:Reason ->
            lager:error("Resize ring request failed ~p:~p", [Exception, Reason]),
...
clear_staged([]) ->
    try
        ...
        Exception:Reason ->
            lager:error("Failed to clear staged cluster changes ~p:~p", [Exception, Reason]),
            io:format("Failed to clear staged cluster changes, see log for details~n"),
            error
    end.

is_claimant_error(Node, Action) ->
    io:format("Failed: ~p is the claimant (see: riak-admin ring_status).~n", [Node]),
    io:format("The claimant is the node responsible for initiating cluster "
              "changes,~nand cannot forcefully ~s itself. You can use 'riak-admin "
              "down' to~nmark the node as offline, which will trigger a new "
              "claimant to take~nover.  However, this will clear any staged "
              "changes.~n",
              [Action]).
...
print_staged([]) ->
    case riak_core_claimant:plan() of
        {error, legacy} ->
            io:format("The cluster is running in legacy mode and does not support "
                      "plan/commit.~n");
        {error, ring_not_ready} ->
            io:format("Cannot plan until cluster state has converged.~nCheck 'Ring "
                      "Ready' in 'riak-admin ring_status'~n");
        {error, invalid_resize_claim} ->
            io:format("Unable to claim some partitions in resized ring.~nCheck that "
                      "there are no pending changes in 'riak-admin ring-status'~nIf "
                      "there are, try again once they are completed~nOtherwise try "
                      "again shortly.~n");
        {ok, Changes, NextRings} ->
            ...
            NextRings2 = lists:sublist(NextRings, erlang:max(0, length(NextRings) - 1)),
...
print_plan(Changes, Ring, NextRings) ->
    ...
    lists:foreach(fun ({Node, join}) ->
                      ...
                      ({_, {resize, NewRingSize}}) ->
                          ...
                          io:format("resize-ring    ~p to ~p partitions~n",
                                    [CurrentSize, NewRingSize]);
                      ({_, abort_resize}) ->
                          CurrentSize = riak_core_ring:num_partitions(Ring),
    ...
    case Transitions of
        1 ->
            io:format("NOTE: Applying these changes will result in 1 cluster transition~n~n");
        _ ->
            io:format("NOTE: Applying these changes will result in ~b cluster transitions~n~n",
                      [Transitions])
    end,

    _ =
        lists:foldl(fun ({Ring1, Ring2}, I) ->
                            io:format("~79..#s~n", [""]),
...
output(Ring, NextRing) ->
    ...
    Reassigned =
        [{Idx, PrevOwner, NewOwner}
         || {{Idx, PrevOwner}, {Idx, NewOwner}} <- Owners3, PrevOwner /= NewOwner],
    ...
    case Reassigned of
        [] ->
            ok;
        _ ->
            io:format("Partitions reassigned from cluster changes: ~p~n", [length(Reassigned)]),
            _ =
    ...
    case {Resizing, Next} of
        ...
        _ ->
            io:format("Transfers resulting from cluster changes: ~p~n", [length(Next)]),
            _ =
...
commit_staged([]) ->
    case riak_core_claimant:commit() of
        ...
        {error, nothing_planned} ->
            io:format("You must verify the plan with 'riak-admin cluster plan' before "
                      "committing~n");
        {error, ring_not_ready} ->
            io:format("Cannot commit until cluster state has converged.~nCheck 'Ring "
                      "Ready' in 'riak-admin ring_status'~n");
        {error, plan_changed} ->
            io:format("The plan has changed. Verify with 'riak-admin cluster plan' "
                      "before committing~n");
        {error, invalid_resize_claim} ->
            io:format("Unable to commit staged ring changes.~nCheck that there are "
                      "no pending changes in 'riak-admin ring-status'~nIf there are, "
                      "try again once they are completed,~nOtherwise try again shortly.~n");
        _ ->
            io:format("Unable to commit cluster changes. Plan may have changed, please "
                      "verify the~nplan and try to commit again~n")
    end.

transfer_limit([]) ->
    {Limits, Down} =
        riak_core_util:rpc_every_member_ann(riak_core_handoff_manager, get_concurrency, [], 5000),
    ...
    lists:foreach(fun (Node) ->
                  ...
                  Down),
    io:format("~79..-s~n", [""]),
    io:format("Note: You can change transfer limits with 'riak-admin transfer_limit "
              "<limit>'~n      and 'riak-admin transfer_limit <node> <limit>'~n"),
    ok;
...
transfer_limit([NodeStr, LimitStr]) ->
    ...
    case Valid of
        false ->
            ...
            error;
        true ->
            case riak_core_util:safe_rpc(Node, riak_core_handoff_manager, set_concurrency, [Limit])
                of
                {badrpc, _} ->
...
security_error_xlate({error, {unknown_users, Names}}) ->
    io_lib:format("User(s) not recognized: ~ts",
                  [string:join(lists:map(fun (X) ->
                                                 unicode:characters_to_list(X, utf8)
                                         end,
                                         Names),
                               ", ")]);
...
security_error_xlate({error, {duplicate_roles, Names}}) ->
    io_lib:format("Ambiguous names need to be prefixed with 'user/' or 'group/': "
                  "~ts",
                  [string:join(lists:map(fun (X) ->
                                                 unicode:characters_to_list(X, utf8)
                                         end,
...
add_role(Name, Options, Fun) ->
    try Fun(Name, parse_options(Options)) of
        ...
        {error, {invalid_option, Option}} ->
            io:format("Invalid option ~p, options are of the form key=value~n", [Option]),
            error
    end.
...
add_source([Users, CIDR, Source | Options]) ->
    ...
    %% sources are as well
    try riak_core_security:add_source(Unames,
                                      parse_cidr(CIDR),
    ...
    of
        ...
        {error, {invalid_option, Option}} ->
            io:format("Invalid option ~p, options are of the form key=value~n", [Option]);
        error:badarg ->
            io:format("Invalid source ~ts, must be latin1, sorry~n", [Source])
    end.
...
grant([Grants, "on", "any", "to", Users]) ->
    grant_int(parse_grants(Grants), any, parse_roles(Users));
grant([Grants, "on", Type, Bucket, "to", Users]) ->
    grant_int(parse_grants(Grants), {Type, Bucket}, parse_roles(Users));
grant([Grants, "on", Type, "to", Users]) ->
    grant_int(parse_grants(Grants), Type, parse_roles(Users));
grant(_) ->
...
revoke([Grants, "on", "any", "from", Users]) ->
    revoke_int(parse_grants(Grants), any, parse_roles(Users));
revoke([Grants, "on", Type, Bucket, "from", Users]) ->
    revoke_int(parse_grants(Grants), {Type, Bucket}, parse_roles(Users));
revoke([Grants, "on", Type, "from", Users]) ->
    revoke_int(parse_grants(Grants), Type, parse_roles(Users));
revoke(_) ->
...
security_status([]) ->
    case riak_core_security:status() of
        ...
        enabled_but_no_capability ->
            io:format("WARNING: Configured to be enabled, but not supported on all "
                      "nodes so it is disabled!~n")
    end.
...
type_status_and_dps([<<"type=", T/binary>> | Rest], _Type, Status, DPs) ->
    NewType =
        case T of
            ...
            _ ->
                try
                    binary_to_existing_atom(T, latin1)
                catch
                    error:_ ->
                        T
                end
...
type_status_and_dps([DPsBin | Rest], Type, Status, DPs) ->
    NewDPs = merge([binary_to_existing_atom(D, latin1) || D <- re:split(DPsBin, ",")], DPs),
...
legacy_search_1(N, Type, Status) ->
    Found =
        exometer_alias:regexp_foldr(N,
                                    fun (Alias, Entry, DP, Acc) ->
                                            orddict:append(Entry, {DP, Alias}, Acc)
                                    end,
                                    orddict:new()),
    lists:foldr(fun ({Entry, DPs}, Acc) ->
                        case match_type(Entry, Type) of
                            true ->
                                ...
                                case exometer:get_value(Entry, DPnames) of
                                    ...
                                    {ok, disabled} when Status == '_'; Status == disabled ->
...
stat_reset(Arg) ->
    lists:foreach(fun ({[{LP, []}], _}) ->
                      ...
                      ({[{LP, Matches}], _}) ->
                          io:fwrite("== ~s (Legacy pattern): ==~n", [LP]),
                          [io:fwrite("~p: ~p~n", [N, exometer:reset(N)]) || {N, _} <- Matches];
                      ({Entries, _}) ->
                          [io:fwrite("~p: ~p~n", [N, exometer:reset(N)]) || {N, _, _} <- Entries]
                  end,
                  find_entries(Arg, enabled)).
...
pick_info_attrs(Arg) ->
    case lists:foldr(fun ("-name", {As, Ps}) ->
                             {[name | As], Ps};
                         ("-type", {As, Ps}) ->
                             {[type | As], Ps};
                         ("-module", {As, Ps}) ->
...
print_info_1(N, [A | Attrs]) ->
    ...
    Body =
        [io_lib:fwrite("~w = ~p~n", [A, proplists:get_value(A, Info)]) | lists:map(fun (value) ->
                                                                                           io_lib:fwrite(Pad
                                                                                                             ++
                                                                                                             "~w = ~p~n",
                                                                                                         [value,
